...
export default class AddFriendsScreen extends React.Component {
  ...
  getUsers = () => {
    ...
    const currentUserId = hotspotsUsers.auth().currentUser.uid;
    const usersRef = hotspotsUsers.database().ref("/users");
    const curUserFriendsRef = hotspotsUsers
      .database()
      .ref("/friends/" + currentUserId);
    var friends = [];
    // Push all of current user's friends to friends array
    curUserFriendsRef
      .once("value")
      .then(user => {
      ...
      .catch(error => this.setState({ error: error.message }));
    // Push all users to users array except for current user and his/her friends
    usersRef.once("value").then(user => {
      var items = [];
      user.forEach(snapshot => {
        if (snapshot.key !== currentUserId && !friends.includes(snapshot.key)) {
          items.push({
  ...
  addFriend = (friendId, email, fName, lName) => {
    this.setState({ loading: true });
    const currentUserId = hotspotsUsers.auth().currentUser.uid;
    ...
    hotspotsUsers
      ...
      .then(() => {
        console.log("Current user's friends list updated");
      })
      .catch(error => this.setState({ error: error.message }));
    ...
    hotspotsUsers
      ...
      .then(() => {
        console.log("Other user's friends list updated");
        this.getUsers();
    ...
    <View style={styles.listItem}>
      ...
      <View style={styles.rightContainer}>
        <Button
          ...
          raised
          onPress={() => {
            this.addFriend(item.friendId, item.email, item.fName, item.lName);
          }}
        />
      </View>
    </View>
